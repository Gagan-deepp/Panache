...
import { armCategory, onlineGames } from "../data";
...
export const fetchEventData = async (category, page = 1, pageSize = 50, filters) => {
    ...
    try {
        ...
        if (filters?.event && filters?.event !== "all") {
            if (onlineGames.includes(filters.event)) {
                // If the event is an online game, filter by eventGame
                query["events.eventGame"] = filters.event;
            } else if (filters?.event === "Arm Wrestling") {
                const allArmWithCategory = armCategory.map((category) => (`Arm Wrestling - ${category}`))
                query["events.eventName"] = { $in: ["Arm Wrestling", ...allArmWithCategory] }
            }
            else {
