...
export const mailToStudent = async (user, group = false) => {
    try {
        const { name, rollno, course, branch, email, phone, events } = user;

        const amount = 50 * events.length;
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <main style="padding: 20px; background-color: #f9f9f9;">
                ...
                <div style="background-color: white; border: 1px solid #ddd; padding: 15px; margin-top: 20px;">
                    ...
                    <ul>
                        ${events.map(event => `<li>${event.eventName}</li>`).join('')}
                    </ul>
                </div>
